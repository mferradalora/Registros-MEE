...
path_csv = "obras_registradas_chile_caudales.csv"

# **2. Normalizador de Texto Mejorado (Ignora tildes, guiones, comillas y símbolos)**
def normalizar_texto(texto):
    ...
    # Reemplazar cualquier símbolo no alfanumérico (apóstrofes, guiones, etc.) por espacios
    texto_str = re.sub(r'[^a-z0-9\s]', ' ', texto_str)
    return ' '.join(texto_str.split())

# **3. Función para filtrar capas vectoriales locales (Soporta todos los tipos de dtype)**
def filtrar_layer_local(gdf, region_nombre):
    ...
    # Iterar por todas las columnas exceptuando la geometría
...
# **4. Cargar y procesar capas nacionales**
...
try:
    ...
    print(f"❌ Error al cargar los archivos. Revisa las rutas: {e}")
...
df_clean = df_csv[
    (df_csv["UTM_Norte"] > 3000000) & (df_csv["UTM_Norte"] < 8000000) &
    (df_csv["UTM_Este"] > 100000) & (df_csv["UTM_Este"] < 900000) &
    (df_csv["Huso"].isin([18, 19]))
].copy()
...
for region_nom in regiones_chile:
    ...
    # CORRECCIÓN CLAVE: show=False evita colapsar el mapa al inicio
...
# **8. Inyectar Leyenda de Simbología**
leyenda_html = """
...
print(f"\n🚀 ¡Éxito! Mapa generado correctamente en '{archivo_salida}'.")
...
# **4. Cargar, procesar y aplicar Jittering de +-5m a obras duplicadas**
print("🔄 Cargando bases de datos nacionales...")
try:
    gdf_regiones_raw = gpd.read_file(carpeta_admin / "Regiones.geojson")
    gdf_comunas_raw = gpd.read_file(carpeta_admin / "Comunas.geojson")
...
# CORRECCIÓN AQUÍ: Convertir explícitamente a float para permitir decimales aleatorios
